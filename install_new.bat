...
If %CurrentBuildNumber% GTR %BuildNumber% (
    ...
    @For /F %%A IN ('wsl -l -v^ | find "1" /C'
        ...
        @For /F %%B IN ('systeminfo | find "System Type"'
        ) Do @If %%B == x64-based (
        ...
        del wsl_update_x64.msi
            ...
            curl -L -C - https://wslstorestorage.blob.core.windows.net/wslblob/wsl_update_arm64.msi --output wsl_update_x64.msi
            msiexec /i "wsl_update_arm64.msi" /passive
            SLEEP 5
            del wsl_update_arm64.msi
        )
        @REM wsl --set-version deck-app 2
    )
    wsl --set-version deck-app 2
) Else (
